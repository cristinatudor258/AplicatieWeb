...
namespace AplicatieWeb.Areas.Identity.Pages.Account.Manage
{
    public class DownloadPersonalDataModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            _logger.LogInformation("Utilizatorul cu ID '{UserId}' si-a cerut datele personale.", _userManager.GetUserId(User));

            // Only include personal data for download
            var personalData = new Dictionary<string, string>();
            var personalDataProps = typeof(AplicatieUtilizator).GetProperties().Where(
